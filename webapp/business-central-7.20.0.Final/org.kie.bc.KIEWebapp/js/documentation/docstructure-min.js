...
}}}function addElement(c,m){var g=c.stencil.id;
var d=false;
if(elementDataInfo[g]){d=true
}else{if(c.properties&&c.properties.tasktype){d=true
}}if(d){var j;
if(g=="Task"){var h=c.properties.tasktype;
if(h=="Send"){j=elementDataInfo.SendTask
}else{if(h=="Receive"){j=elementDataInfo.ReceiveTask
}else{if(h=="Manual"){j=elementDataInfo.ManualTask
}else{if(h=="Service"){j=elementDataInfo.ServiceTask
}else{if(h=="Business Rule"){j=elementDataInfo.BusinessRuleTask
}else{if(h=="Script"){j=elementDataInfo.ScriptTask
}else{if(h=="None"){j=elementDataInfo.Task
}else{if(h=="User"){j=elementDataInfo.UserTask
}}}}}}}}}else{if(c.properties&&c.properties.tasktype){j=elementDataInfo.WorkItem
}else{j=elementDataInfo[g]
}}var l=[];
var k="";
var f=j.properties.split(",");
for(var b=0;
b<f.length;
b++){var a=f[b];
if(a=="name"){k=c.properties[a]
}l.push({name:presentPropertyName(a),value:parsePropertyValue(a,c.properties[a]).replace(/</g,"&lt;").replace(/>/g,"&gt;")})
}var e=true;
if(j.group=="ConnectingObjects"){if(!c.properties.name.length&&!c.properties.documentation.length&&!c.properties.conditionexpression.length){e=false
}}m[j.group].push({id:c.resourceId,group:j.group,groupdispname:j.groupdispname,dispname:j.dispname,groupicon:ctx+j.groupicon,icon:ctx+j.icon,properties:l,nodename:k.trim().length>0?k:"(Not Named)",showindocumentation:e})
...
if(l[1]!="false"&&l[1]!="true"){c+=l[0]+"("+l[1]+")\n"
}else{c+=l[0]+"(no defined type)\n"
...
}}function displayProcessImg(){var f=parent.ORYX.EDITOR.getCanvas().getSVGRepresentation(true,true);
var d=parent.DataManager.serialize(f);
var g=f.getAttributeNS(null,"height");
var j=f.getAttributeNS(null,"width");
var c=Object.hasOwnProperty.call(window,"ActiveXObject");
var a=/Chrome/.test(navigator.userAgent)&&/Google Inc/.test(navigator.vendor);
if(a||c){var b=document.getElementById("processimgdivdisplay");
...
}else{var e=document.getElementById("processimgdivdisplayframe");
...
},params:{profile:parent.ORYX.PROFILE,uuid:window.btoa(encodeURI(parent.ORYX.UUID)),fsvg:parent.Base64.encode(d),transformto:"png",respaction:"showurl",svgheight:g,svgwidth:j}})
}}function showElementInModel(a){parent.ORYX.EDITOR._pluginFacade.raiseEvent({type:parent.ORYX.CONFIG.EVENT_DOCELEMENT_TO_MODEL,eleid:a});
...
}function scrollToElement(){if(parent.ORYX.PROCESSDOC_RESOURCEID!=""&&parent.ORYX.PROCESSDOC_RESOURCEID.length>0){document.getElementById(parent.ORYX.PROCESSDOC_RESOURCEID).scrollIntoView();
