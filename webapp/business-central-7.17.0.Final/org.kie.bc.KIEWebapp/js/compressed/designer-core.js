...
},updateViewLockState:function(b){if(ORYX.INSTANCE_VIEW_MODE!=true){if((typeof parent.isLocked==="function")&&(typeof parent.isLockedByCurrentUser==="function")){var a=parent.isLocked();
var c=parent.isLockedByCurrentUser();
var d=(ORYX.READONLY==true)||(ORYX.VIEWLOCKED==true);
if(!a){ORYX.VIEWLOCKED=false
}else{if(a&&!c){ORYX.VIEWLOCKED=true
}else{if(a&&c){ORYX.VIEWLOCKED=false
}}}if(d&&!ORYX.VIEWLOCKED&&b){if(typeof parent.reload==="function"){ORYX.PROCESS_SAVED=true;
parent.reload()
...
},_handleEditProps:function(){if(ORYX.CONFIG.PANEL_RIGHT_COLLAPSED_SWITCH==true){this.layout_regions.east.expand()
}else{this.layout_regions.east.collapse()
}ORYX.CONFIG.PANEL_RIGHT_COLLAPSED_SWITCH=!ORYX.CONFIG.PANEL_RIGHT_COLLAPSED_SWITCH
},_handleEditMenu:function(){if(ORYX.CONFIG.PANEL_LEFT_COLLAPSED_SWITCH==true){this.layout_regions.west.expand()
}else{this.layout_regions.west.collapse()
...
Ext.getCmp("maintabs").remove("processdoctab")
}if(ORYX.BPSIMDISPLAY!=true){Ext.getCmp("maintabs").remove("simulationtab")
...
return b
}},resetAllShapeColors:function(){this.getCanvas().children.each(function(a){this.resetShapeColors(a)
}.bind(this));
this.getCanvas().update()
},resetShapeColors:function(a){if(a){if(a instanceof ORYX.Core.Node||a instanceof ORYX.Core.Edge){a.setProperty("oryx-bordercolor",a.properties["oryx-origbordercolor"]);
a.refresh()
}if(a.getChildren().size()>0){for(var b=0;
b<a.getChildren().size();
b++){if(a.getChildren()[b] instanceof ORYX.Core.Node||a.getChildren()[b] instanceof ORYX.Core.Edge){this.resetShapeColors(a.getChildren()[b])
...
}else{if(ORYX.PROCESS_SAVED===undefined){return true
...
}if(!c.boundaryonly){c.boundaryonly=false
...
},boundaryonly:function(){return this._jsonProp.boundaryonly
...
},setBoundaryonly:function(){this._jsonProp.boundaryonly=true
...
$A(i.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_SVG,"a")).each(function(e){if(e.getAttributeNS("http://www.w3.org/1999/xlink","href")&&e.getAttributeNS("http://www.w3.org/1999/xlink","href")!==null){e.setAttributeNS("http://www.w3.org/1999/xlink","xlink:href",e.getAttributeNS("http://www.w3.org/1999/xlink","href").escapeHTML())
}});
return i
...
c[h.id]=h
}if(ORYX.Editor.checkClassType(h,SVGImageElement)){if(h.getAttributeNS("http://www.w3.org/1999/xlink","href")&&h.getAttributeNS("http://www.w3.org/1999/xlink","href")!==null){h.setAttributeNS("http://www.w3.org/1999/xlink","href",h.getAttributeNS("http://www.w3.org/1999/xlink","href"))
}}}).bind(this))
}).bind(this))
}else{d.refToView().each((function(i){if(i===""){this.propertiesChanged[b.key]=true;
return
}var k=this.id+i;
var h=this.node.ownerDocument.getElementById(k);
if(!h||!(h.ownerSVGElement)){if(d.type()===ORYX.CONFIG.TYPE_URL||d.type()===ORYX.CONFIG.TYPE_DIAGRAM_LINK){var m=this.node.ownerDocument.getElementsByTagNameNS("http://www.w3.org/2000/svg","a");
h=$A(m).find(function(n){return n.getAttributeNS(null,"id")===k
});
if(!h){this.propertiesChanged[b.key]=true;
return
}}else{this.propertiesChanged[b.key]=true;
return
}}if(d.complexAttributeToView()){var g=this._labels[k];
if(g){try{propJson=e.evalJSON();
var j=propJson[d.complexAttributeToView()];
g.text(j?j:e)
}catch(l){g.text(e)
}}}else{switch(d.type()){case ORYX.CONFIG.TYPE_BOOLEAN:if(typeof e=="string"){e=e==="true"
}h.setAttributeNS(null,"display",(!(e===d.inverseBoolean()))?"inherit":"none");
break;
case ORYX.CONFIG.TYPE_COLOR:if(d.fill()){if(h.tagName.toLowerCase()==="stop"){h.setAttributeNS(null,"stop-color",e);
if(h.parentNode.tagName.toLowerCase()==="radialgradient"){ORYX.Utils.adjustGradient(h.parentNode,h)
}}else{h.setAttributeNS(null,"fill",e)
}}if(d.stroke()){h.setAttributeNS(null,"stroke",e);
if(h.tagName.toLowerCase()==="stop"){h.setAttributeNS(null,"stop-color",e);
if(h.parentNode.tagName.toLowerCase()==="radialgradient"){ORYX.Utils.adjustGradient(h.parentNode,h)
}}}break;
case ORYX.CONFIG.TYPE_STRING:var g=this._labels[k];
if(g){g.text(e)
}break;
case ORYX.CONFIG.TYPE_STRING:var g=this._labels[k];
if(g){g.text(e)
}break;
case ORYX.CONFIG.TYPE_TEXT:var g=this._labels[k];
if(g){g.text(e)
}break;
case ORYX.CONFIG.TYPE_FLOAT:if(d.fillOpacity()){h.setAttributeNS(null,"fill-opacity",e)
}if(d.strokeOpacity()){h.setAttributeNS(null,"stroke-opacity",e)
}if(!d.fillOpacity()&&!d.strokeOpacity()){var g=this._labels[k];
if(g){g.text(e)
}}break;
case ORYX.CONFIG.TYPE_URL:break;
case ORYX.CONFIG.TYPE_CALLEDELEMENT:break;
case ORYX.CONFIG.TYPE_DIAGRAM_LINK:var f=h.getAttributeNodeNS("http://www.w3.org/1999/xlink","xlink:onclick");
if(f){f.textContent=e
}else{h.setAttributeNS("http://www.w3.org/1999/xlink","xlink:onclick",e)
}break
}}}).bind(this))
}}}).bind(this));
